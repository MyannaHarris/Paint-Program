...
// red,green,blue,magenta,black,pen,brush,roller,clear,eraser,spray,band,small,medium,large --> buttons
...
// oldX,oldY,newX,newY,x,y,startX,startY,endX,endY --> represents the continuous x and y values
...
// pannel, drawArea --> boundary areas
// color --> represents if there is color or it is set to white for eraser
// size, s --> represent the sizes of the eraser and spray paint
// penta --> represents the object that draws pentagons
// rnd --> the random variable that picks a random number for spray paint
// rubberBand --> represents wether or not the user is using the rubber band line drawer
// ***********************************************
// Lab13.java
// 110 Version
...
public class Lab13 extends Applet
{
	...
	public void init()
    {
		...
		clear   = new Rectangle(10,220,50,25);
		numDraw = 0;
		...
		//area where user can draw in (stops user from drawing on and around tools)
		drawArea = new Rectangle(73,0,appletWidth-73,appletHeight);
   	}
   ...
   public boolean mouseDown(Event e, int x, int y)
	{
		...
		else if(band.inside(x,y))
		{
			...
			size = 3;
		else if(drawArea.inside(x,y))	//default to drawing when a button isn't clicked
		{
			newX = x;
			newY = y;
			oldX = newX;
	...
	{
		if ((rubberBand == true)&&(drawArea.inside(x,y)))
		{
			endX = x;
			endY = y;
		}
